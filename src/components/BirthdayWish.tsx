export function BirthdayWish() {
  return (
    <section className="py-16 px-4">
      <div className="container mx-auto max-w-4xl">
        <div className="birthday-card text-center scale-in">
          <h2 className="text-4xl md:text-5xl font-dancing font-bold gradient-text mb-8">
            A Special Wish for You
          </h2>
          <div className="space-y-6 text-lg md:text-xl leading-relaxed">
            <p className="font-medium">
              On this beautiful day, we want you to know how truly special you are to all of us.
            </p>
            <p>
              Your Smile fills contigious to everbody, Look like angel from heavean.Our college journey is incomplete without you, 
              and your relationship, is a gift we treasure every single day.
            </p>
            <p>
              As you celebrate another year of life, may you be surrounded by lovely Friends, 
              blessed with happiness, and filled with excitement for all the wonderful 
              adventures that lie ahead.
            </p>
            <p className="text-2xl font-dancing font-semibold gradient-text">
              You deserve all the magic this world has to offer! ✨
            </p>
          </div>
        </div>
      </div>
    </section>
  )
}